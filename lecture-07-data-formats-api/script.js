...
// ===================================================
// Comments on JS Async
// ===================================================

/* Why do we use async/await?
We use async/await to handle asynchronous operations (like fetch) in a cleaner and more readable way.
It allows us to wait for the server response before continuing the code,
instead of using complex .then() chains.

Why do we check response.ok?
We check response.ok to make sure the HTTP request was successful (status 200–299).
If we don't check it, the code might try to process invalid data
even if the server returned an error like 404 or 500.

Why do we use try/catch?
We use try/catch to handle possible errors, such as network problems
or issues while converting the response to JSON.
It prevents the program from crashing and allows us to display an error message to the user. */